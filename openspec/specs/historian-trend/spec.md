# historian-trend Specification

## Purpose
定义 Industrial HMI 第一阶段历史数据和趋势分析能力，包括 Main Process SQLite Historian、Tag History 写入策略、实时趋势 Ring Buffer、历史趋势查询与聚合，以及 Renderer 趋势状态的数据量边界。
